...
def use_case():
    ...
    d.textblock(755, 102, "Hệ thống Quản lý Thư viện", 21, True, "#333")
    # actors
    d.actor(110, 360, "Bạn đọc\n(SV/GV/NCS)")
    d.actor(1410, 300, "Thủ thư")
    d.actor(1410, 870, "Quản trị viên")
    d.actor(110, 960, "Bộ lập lịch\n(Scheduler)")
    EW, EH = 312, 70
    # reader use cases
    rx = 560
    R = [("Tra cứu & xem chi tiết TL", 165),
         ("Quản lý giỏ & tạo đơn mượn", 285),
         ("Xác nhận nhận sách (nhập mã)", 405),
         ("Gia hạn / Tự huỷ đơn chờ lấy", 525),
         ("Xem & đánh dấu thông báo", 645),
         ("Đăng nhập / Đổi mật khẩu", 765)]
    for t, y in R:
        d.ellipse(rx, y, EW, EH, t, fill=C_READER)
        d.arrow(155, 362, rx-EW/2+8, y, head=False, width=2)
    # librarian use cases
    lx = 945
    L = [("Chuẩn bị & giao sách theo mã", 165),
         ("Trả sách & tính/thu phí phạt", 285),
         ("Huỷ đơn / Cho mượn đọc tại chỗ", 405),
         ("Quản lý sách (CRUD + tồn kho)", 525),
         ("Cấu hình quy tắc & hiển thị loại TL", 645)]
    for t, y in L:
        d.ellipse(lx, y, EW, EH, t, fill=C_LIB)
        d.arrow(1365, 302, lx+EW/2-8, y, head=False, width=2)
    # admin use cases
    A = [("Quản lý tài khoản & khoá/mở thẻ", 815),
         ("Cấu hình ma trận phân quyền", 935)]
    for t, y in A:
        d.ellipse(lx, y, EW, EH, t, fill=C_ADMIN)
        d.arrow(1365, 872, lx+EW/2-8, y, head=False, width=2)
    # scheduler use cases
    Suc = [("Tự huỷ đơn quá hạn lấy (24h)", 905),
           ("Nhắc hạn trả (sắp/đã quá hạn)", 1025)]
    for t, y in Suc:
        d.ellipse(rx, y, EW, EH, t, fill=C_SYS)
        d.arrow(155, 962, rx-EW/2+8, y, head=False, width=2)
    d.save("/tmp/uml_usecase.png")
...
def activity():
    ...
    act(150, "Đăng nhập (Spring Security, BCrypt)")
    d.arrow(cx, 193, cx, 250)
    act(290, "Tra cứu tài liệu\n(chỉ loại được phép xem)")
    d.arrow(cx, 333, cx, 390)
    act(430, "Thêm sách vào giỏ")
    d.arrow(cx, 473, cx, 535)
    dec(610, "Qua 6 lớp kiểm tra giỏ?\n(quyền · trùng · ≤5 ·\nkhông nội sinh · được phép ·\ncòn sách)")
    # fail -> back to add
    d.box(fx-150, 610-43, 300, 86, "Báo lỗi, giữ nguyên giỏ", 15, fill=C_FAIL, radius=14)
    d.arrow(cx+DW/2, 610, fx-150, 610, label="Không")
    d.arrow(fx, 610-43, fx, 430, head=True)
    d.arrow(fx, 430, cx+BW/2, 430, head=True)
    d.arrow(cx, 610+DH/2, cx, 745, label="Có", ldx=22)
    act(785, "Gửi phiếu mượn (submit)")
    d.arrow(cx, 828, cx, 885)
    dec(965, "RuleEngine validate 3 tầng?\n(thẻ khoá · đang quá hạn ·\nvượt hạn mức theo loại)")
    d.box(fx-150, 965-43, 300, 86, "Hiển thị lỗi\n(thông báo tiếng Việt)", 15, fill=C_FAIL, radius=14)
    d.arrow(cx+DW/2, 965, fx-150, 965, label="Không")
    d.arrow(fx, 965+43, fx, 1120)
    d.arrow(fx, 1120, cx+BW/2, 1120, head=True)
    d.arrow(cx, 965+DH/2, cx, 1085, label="Có", ldx=22)
    d.box(cx-BW/2, 1085-50, BW, 100, "Tạo đơn AWAITING_PICKUP\n+ sinh mã 6 ký tự + giữ chỗ (copies−1)\n+ gửi thông báo", 14, fill=C_SYS, radius=14)
    d.arrow(cx, 1135, cx, 1195)
    dec(1280, "Bạn đọc đến lấy &\nnhập đúng mã trong 24h?")
    # timeout branch
    d.box(fx-160, 1280-58, 320, 116, "Scheduler tự huỷ đơn\n(quá 24h chưa lấy)\nhoàn trả số lượng → CANCELLED", 14, fill=C_FAIL, radius=14)
    d.arrow(cx+DW/2, 1280, fx-160, 1280, label="Không / quá hạn")
    d.end(fx, 1480); d.arrow(fx, 1280+58, fx, 1480-15)
    d.arrow(cx, 1280+DH/2, cx, 1410, label="Có", ldx=22)
    d.box(cx-BW/2, 1410-50, BW, 100, "confirmPickup → BORROWED\nĐặt hạn trả theo vai trò × loại TL\n(có thể Gia hạn nếu còn lượt)", 14, fill=C_LIB, radius=14)
    d.arrow(cx, 1460, cx, 1520)
    d.box(cx-BW/2, 1520-43, BW, 86, "Thủ thư nhận trả → tính phí\n(ngày trễ × 2.000đ) → thu phí", 15, fill=C_LIB, radius=14)
    d.arrow(cx, 1563, cx, 1615)
    dec(1690, "Mọi cuốn đã trả\n& mọi phí đã thu?")
    # loop back if not
    d.arrow(cx-DW/2, 1690, 150, 1690, label="Chưa")
    d.arrow(150, 1690, 150, 1520, head=False); d.arrow(150, 1520, cx-BW/2, 1520, head=True)
    d.arrow(cx, 1690+DH/2, cx, 1775, label="Có", ldx=22)
...
def state_machine():
    ...
    d.arrow(133, 180, 360-SW/2, 180, label="submitLoan /\ntạo mã + giữ chỗ", ldy=-28)
    d.arrow(360+SW/2, 180, 820-SW/2, 180, label="confirmPickup [mã đúng] /\nđặt hạn trả", ldy=-28)
    d.arrow(820+SW/2, 180, 1280-SW/2, 180, label="đã trả hết &\nthu hết phí /\nrefreshCompletion", ldy=-36)
    ...
    d.label(820, 74, "renewBook [còn lượt] / +renewalDays", 13)
    ...
    d.label(820, 318, "returnBook / tính phí + tăng copies", 13)
    # COMPLETED final
    ...
    d.label(610, 370, "huỷ (bạn đọc / thủ thư)\nhoặc quá 24h (scheduler) /\nhoàn trả copies", 13)
    d.end(360, 740); d.arrow(360, 560+SH/2, 360, 740-15)
    # LoanDetail lifecycle strip (bottom)
    d.textblock(235, 900, "Vòng đời\nLoanDetail:", 16, True, "#333")
    for t, x in [("RESERVED", 640), ("BORROWING", 920), ("RETURNED", 1200)]:
        d.box(x-120, 900-30, 240, 60, t, 15, fill="#EEEEEE", radius=14)
    d.arrow(640+120, 900, 920-120, 900, label="lấy sách", ldy=-20, lsize=12)
    d.arrow(920+120, 900, 1200-120, 900, label="trả sách", ldy=-20, lsize=12)
    d.label(920, 968, "(OVERDUE = tính động khi dueDate < hôm nay; không lưu thành trạng thái riêng)", 12, "#666")
    d.save("/tmp/uml_state.png")
...
def class_diagram():
    ...
    d.textblock(775, 825, "BorrowingRule khoá theo (userRole, docType);\nRolePermission khoá theo (userRole, permission).\nHai bảng cấu hình tra cứu, không gắn khoá ngoại tới User.", 14, fill="#7a5c00")
    d.save("/tmp/uml_class.png")


use_case(); activity(); state_machine(); class_diagram()
print("done: usecase, activity, state, class")
for f in ["usecase","activity","state","class"]:
    im = Image.open(f"/tmp/uml_{f}.png"); print(f, im.size)
